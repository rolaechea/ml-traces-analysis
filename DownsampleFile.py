...
# -*- coding: utf-8 -*-
"""
Created on Tue Nov 13 13:55:52 2018
...
downSampleToNewMaxExecutions, extractLinearArrayTimeTakenForSingleTransition
...
if __name__ == "__main__":
    downsampleAndMerge = False
    writeDownsampledOutput = False
    outFilename = ""
    ...
    if  len(sys.argv) > 1:
         ...
         if  len(sys.argv) > 2:
             if sys.argv[2] == "downmerge":
                downsampleAndMerge = True
                 
                if  len(sys.argv) > 3:
                     writeDownsampledOutput = True
                     outFilename = sys.argv[3]
                else:
                    print(" Invalid Usage. Required Usage: {0}   pickledFilename [downmerge outputFilename]".format(sys.argv[0]))
                    exit(0)                
    else:        
        print(" Invalid Usage. Required Usage: {0}   pickledFilename [downmerge outputFilename]".format(sys.argv[0]))
        exit(0)
...
#    print("Transition Counts Before Resampling: \n {0} \n".format(allCounts))
    if downsampleAndMerge == True:
        ...
        finalArrayDict = transitionArrayOfDictionary
        
        allCounts = calculatePerTransitionsCounts(finalArrayDict)
        
    print(allCounts)

    for transitionId in allCounts.keys():
         X_Train = trainingSetConfigurations
         Y_Train = extractLinearArrayTimeTakenForSingleTransition(finalArrayDict, transitionId)
    
    if writeDownsampledOutput:
        output = open(outFilename, 'wb')
        pickle.dump(finalArrayDict, output, pickle.HIGHEST_PROTOCOL)
        output.close()
#         print(" Transition {0}, Size X_Train = {1}, Size Y_Train = {2}".format(transitionId, len(X_Train), len(Y_Train)))